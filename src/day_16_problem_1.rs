...
impl PacketStreamer {
    ...
    fn parse_packet(&mut self) -> (PacketType, u64) {
        ...
        let packet_type = convert_bit_array_to_number(self.get_n_bits(3));
        let mut bits_used = 6;
        match packet_type {
            4 => {
                ...
                (PacketType::Literal(version, value), bits_used)
            }
            _ => {
                ...
                if length_type == 0 {
                    ...
                    (PacketType::Operator(version, subpackets), bits_used)
                } else {
    ...
    fn get_next_packet(&mut self) -> Option<PacketType> {
        ...
        match packet_type {
            4 => {
                ...
                Some(PacketType::Literal(version, value))
            }
            _ => {
                let length_type = self.get_n_bits(1).pop_front().unwrap_or(0);
                bits_taken += 1;
                if length_type == 0 {
                    ...
                    Some(PacketType::Operator(version, subpackets))
                } else {
...
enum PacketType {
    ...
    Operator(Version, Vec<PacketType>),
}
...
fn get_sum_of_packet_versions(packet: PacketType) -> u64 {
    match packet {
        PacketType::Literal(version, _) => version,
        PacketType::Operator(version, subpackets) => {
            let mut total = version;
            for subpacket in subpackets {
                total += get_sum_of_packet_versions(subpacket);
            }
            total
        }
    }
...
pub fn day_16_problem_1() -> io::Result<u64> {
    ...
    for packet in packet_streamer {
        total += get_sum_of_packet_versions(packet)
    }
    Ok(total)
}
