...
pub fn day_18_problem_1() -> io::Result<i64> {
    ...
    let mut sum = flatten(&parse_input(String::from(first_line)));
    for line in lines {
        let next_to_add = flatten(&parse_input(String::from(line)));
        let new_sum = add(sum, next_to_add);
        sum = new_sum;
    }

    println!(
        "{:?}",
        sum.iter().map(|(_, val)| *val).collect::<Vec<i64>>()
    );

    let mut mag = magnitude(sum);
